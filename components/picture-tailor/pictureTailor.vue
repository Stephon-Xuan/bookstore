<template>
	<view class="picture-tailor" :class="{ 'picture-tailor-show': isShow }">
		...
		<!-- #endif -->
		<!-- #ifdef APP-PLUS -->
		<button class="button-view" @click="AppreateImg">ok</button>
		<!-- #endif -->
...
export default {
	...
	data() {
		return {
			...
			img_width: 400, // 图片真实宽度
			img_height: 400, // 图片真实高度
		...
		getImgInfo() {
			uni.getImageInfo({
				...
				success: res => {
					...
					// 计算初始缩放比和最小缩放值
					if (this.img_width < tailorSize || this.img_height < tailorSize) {
						// 当图片宽或高小于240px时
						let count = this.img_width <= this.img_height ? this.img_width : this.img_height;
						this.img_scaling = count / tailorSize;
						this.scaleMin = 1;
					} else if (this.img_width > windowWidth && this.img_width <= this.img_height) {
						// 当图片宽度大于屏幕宽度并且图片宽度小于图片高度时
			...
			ctx.draw(false, function (){
				...
				uni.canvasToTempFilePath({
					...
					success: res => {
						...
						// this.onHide();
						// this.$emit("getAvatorImg", res.tempFilePath)
						uni.setStorageSync('AvatorImg', res.tempFilePath);
						uni.navigateBack({});
					},
					fail: res => {
